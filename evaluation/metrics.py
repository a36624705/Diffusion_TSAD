...
def get_metrics(score, labels, slidingWindow=100, pred=None, version='opt', thre=250):
    ...
    if score is None:
        AUC_ROC, AUC_PR, VUS_ROC, VUS_PR, PointF1PA = None, None, None, None, None
    else:
    ...
    _, _, PointF1 = grader.metric_PointF1(labels, score, preds=pred)
    _, _, EventF1PA = grader.metric_EventF1PA(labels, score, preds=pred)
    _, _, RF1 = grader.metric_RF1(labels, score, preds=pred)
    P_a, R_a, Affiliation_F = grader.metric_Affiliation(labels, score, preds=pred)
    ...
    metrics['Affiliation-P'] = P_a
    metrics['Affiliation-R'] = R_a
    return metrics